...
let readFile = (arcI, imagenUrl, mensaje) => {
  return new Promise((resolve, reject) => {
    fs.readFile(arcI, "utf8", function (err, data) {
      ...
      let eleme = dataArray.find(
        (Element) => Element === "Name,Phone 1  Value"
      );
      ...
      dataArray.map((index, position) => {
        ...
        if (number == false || name == false) {
          ...
          createSms(name, number, imagenUrl, mensaje)
            .then((data) => {
              console.log("por aca todo bien"),
                (sendMessages = sendMessages + 1);
            })
            .catch((err) => {
              console.log("por aca no se envio nada "), sendFail.push(index);
            });
        ...
        if (position === dataArray.length - 1) {
          resolve((data) => {
            return {
              "send messages total": sendMessages,
              "sen messages": indexSend,
              sendFail: sendFail,
            };
          });
          fs.unlinkSync(arcI);
